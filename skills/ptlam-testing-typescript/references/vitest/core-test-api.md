...
Vitest exposes `test` and `it` as equivalent case-definition APIs. This skill
selects `describe`/`it`: author cases and modifiers as `it`, `it.each`,
`it.concurrent`, and so on. The retained upstream examples may use `test` only
to preserve complete Vitest API context; translate that alias to `it` in project
code.
...
See [features-context](features-context.md) for fixture scopes, `test.override`, and the object syntax.
