...
This skill prefers `toMatchSnapshot` for complex stable outputs. When output
varies by language, use awaited `toMatchFileSnapshot` with an explicit relative
path so each language owns a separate golden file.
...
- Use `toMatchSnapshot` for complex outputs whose complete structure matters
- Use awaited `toMatchFileSnapshot` with an explicit path for language-specific
  outputs
