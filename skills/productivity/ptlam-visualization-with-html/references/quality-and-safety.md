...
- Keep every essential conclusion and material caveat available without
  JavaScript. See [visualization.md](visualization.md) for visual alternatives.
...
- Follow the requested path, then an existing project artifact convention, then
  `.ptlam/ptlam-visualization-with-html/<descriptive-name>.html`. Do not
  silently overwrite an unrelated file.
