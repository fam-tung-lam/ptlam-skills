...
const designSystem = new URL(
  "../../../../skills/productivity/ptlam-visualization-with-html/assets/design-system/",
  import.meta.url,
);
...
test("the starter is semantic, complete, and safe to assemble", () => {
  ...
  assert.match(
    template,
    /<meta\s+name="generator"\s+content="ptlam-visualization-with-html"\s*\/>/,
  );
  assert.match(
    template,
    /<meta\s+name="ptlam-visualization-with-html-version"\s+content="1"\s*\/>/,
  );
...
test("bundled sources are original and request no remote dependency", () => {
  ...
  assert.match(icons, /Original ptlam-visualization-with-html icon sprite/);
