...
const validatorPath = join(
  repositoryRoot,
  'skills/productivity/ptlam-visualization-with-html/scripts/validate.mjs',
);
...
async function withTemporaryDirectory(run) {
  const directory = await mkdtemp(join(tmpdir(), 'ptlam-visualization-with-html-validator-'));
  try {
...
test('missing input and malformed required structure are actionable errors', async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    assert.match(malformed.stdout, /^ERROR \[metadata-ptlam-visualization-with-html-version\]/m);
  });
});
...
test('unsafe opener semantics and broken local assets are errors', async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    assert.match(result.stdout, /^ERROR \[external-link-opener\].*noopener noreferrer/m);
...
<html lang="not_a_tag">
  <head>
    ...
    <meta name="ptlam-visualization-with-html-version" content="2">
    <title> </title>
  </head>
  <body><main>Readable source</main></body>
    ...
    assert.match(result.stdout, /^ERROR \[metadata-viewport\].*width=device-width/m);
    assert.match(result.stdout, /^ERROR \[metadata-generator\]/m);
    assert.match(result.stdout, /^ERROR \[metadata-ptlam-visualization-with-html-version\]/m);
  });
});
...
test('asset paths outside the artifact directory are rejected without following them', async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    const result = await runValidator([htmlPath]);
    assert.notEqual(result.code, 0);
    assert.match(result.stdout, /^ERROR \[asset-outside-context\].*was not followed/m);
...
test('remote dependencies and size thresholds are warnings and preserve a zero exit', async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    assert.equal(result.code, 0, result.stdout + result.stderr);
    assert.doesNotMatch(result.stdout, /^ERROR /m);
    assert.match(result.stdout, /^WARNING \[remote-dependency\].*it was not fetched/m);
    assert.equal((result.stdout.match(/^WARNING \[asset-size\]/gm) ?? []).length, 2);
    assert.match(result.stdout, /^WARNING \[total-size\]/m);
    assert.match(result.stdout, /^SUMMARY errors=0 warnings=4 unverified=5 local-assets=2/m);
  });
});
...
test('complex source syntax is unverified instead of guessed', async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    const result = await runValidator([htmlPath]);
    assert.equal(result.code, 0, result.stdout);
    assert.match(result.stdout, /^UNVERIFIED \[source-syntax\].*complex or ambiguous/m);
    assert.doesNotMatch(result.stdout, /^ERROR \[(?:missing-asset|asset-reference)\]/m);
  });
});
...
test('invalid UTF-8 and invalid CLI usage fail before browser claims are emitted', async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    assert.match(invalidUtf8.stdout, /^ERROR \[input-readability\].*not valid UTF-8/m);
