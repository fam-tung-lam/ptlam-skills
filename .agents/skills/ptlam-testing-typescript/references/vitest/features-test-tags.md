...
export default defineConfig({
  test: {
    tags: [
      { name: 'unit', description: 'Pure unit tests.' },
...
declare module 'vitest' {
  interface TestTags {
    tags: 'unit' | 'integration' | 'db' | 'flaky'
  }
}
```
...
test('parses a valid record', { tags: ['unit'] }, () => {})
...
Programmatic: pass `tagsFilter: ['unit and db']` to `startVitest`/`createVitest`.
...
- Use tags for cross-cutting categories with shared timeout/retry; use [projects](advanced-projects.md) for different pools or configurations
