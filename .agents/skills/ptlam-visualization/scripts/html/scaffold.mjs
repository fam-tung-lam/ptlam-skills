...
function usage() {
  return (
    "Usage: node skills/productivity/ptlam-visualization/scripts/html/scaffold.mjs " +
...
export async function assembleDocument({
  ...
  if (typeof title !== "string" || title.trim() === "") {
    throw new ScaffoldError(
      "assembly",
      "Document title must be a non-blank string.",
    );
  }
  if (typeof lang !== "string" || !languageTagPattern.test(lang)) {
    throw new ScaffoldError(
      "assembly",
      "Document language must be a well-formed language tag.",
    );
  }
  if (typeof trustedContent !== "string") {
    throw new ScaffoldError(
      "assembly",
      "Trusted content must be an HTML string.",
    );
  }
  if (capability !== "html" && capability !== "combined") {
    throw new ScaffoldError(
      "assembly",
      'Artifact capability must be "html" or "combined".',
    );
  }
  if (capability === "html" && trustedContent !== "") {
    throw new ScaffoldError(
      "assembly",
      "The public HTML scaffold does not accept injected content.",
    );
  }
  if (capability === "combined") {
    const { parseEmbeddedMermaidRecords } =
      await import("./lib/embedded-mermaid-record.mjs");
    await parseEmbeddedMermaidRecords(trustedContent);
  }
  ...
  document = replaceSlot(document, "CONTENT", trustedContent);
