...
- Made the `ptlam-visualization-with-html` design-system references
  ...
  external pages.
- Reduced `ptlam-git` to portable commit-message preferences and project-local
  `CONTEXT.md` management, relying on agents' existing Git workflow knowledge.
...
- Refined `ptlam-creating-atomic-note` with explicit operation outcomes, safe
  merge-file handling, syntax-neutral connection fallbacks, and all atomic-note
  principles consolidated in the main skill instructions.
