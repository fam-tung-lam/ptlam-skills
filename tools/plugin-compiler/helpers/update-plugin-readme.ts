...
import type { ManifestSkill } from "../models/skill.ts";
...
function renderRootCatalogSection(plugin: PluginModel): string {
...
function renderCategorySection(plugin: PluginModel): string {
 ...
 */
export function updatePluginReadme({
  plugin,
  rootReadme,
  skillsReadme,
}: {
  ...
  rootReadme: string;
  skillsReadme: string;
}): { rootReadme: string; skillsReadme: string } {
  return {
